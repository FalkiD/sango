...
module opcodes #(parameter MMC_FILL_LEVEL_BITS = 16,
  ...
  (
    ...
    reg  [7:0]   rsp_data [15:0];    // 64k array of response bytes (echo, status)
...
//   0, 0, 0, ECHO, PAINTFCFG, SYNCCONF, TRIGCONF, LENGTH, MODE, BIAS, PULSE, PHASE, POWER, FREQ, STATUS, TERMINATOR
    ...
    always @( posedge sys_clk) begin
        if(!sys_rst_n) begin
            ...
            if(operating_mode == `PTNCMD_RUN && ptn_data_i != `PTNDATA_NONE) begin
...
//                end
                ...
                `STATE_WMD: begin
                  ...
                  // Wait Measurement Delay, wait until pulse processor
                  // completes so all measurements are done
                  if(pulse_busy_i == 1'b0) begin
    ...
    //                            end
            ...
            // ng cuz variable width??? uinttmp <= {fifo_dat_i, uinttmp[63:8]};
