...
--end process;
  ...
  opcode_processor : opcodes
  generic map(
    RSP_FILL_LEVEL_BITS   => MMC_FILL_LEVEL_BITS -- Should be at least int(floor(log2(FIFO_DEPTH))+1.0)
  )
  port map (
    clk               => sys_clk,
    rst_n             => sys_rst_n,
    ...
    fifo_dat_i        => s_fif_dat_rd,              -- opcode fifo
    fifo_rd_en_o      => s_fif_rd,                  -- fifo read line
    fifo_rd_empty_i   => s_fif_rd_empty,            -- fifo empty flag
    fifo_rd_count_i   => s_fif_dat_rd_level,        -- fifo fill level 
  
    system_state_i    => opc_system_state,          -- overall state of system, "running a pattern", for example
    mode_o            => opc_system_mode,           -- MODE opcode can set system-wide flags
  
    response_o              => s_fif_wr,            -- to fifo, response bytes(status, measurements, echo, etc)
    response_wr_en_o        => s_fif_we,            -- response fifo write enable
    response_fifo_empty_i   => s_fif_wr_empty,      -- response fifo empty flag
    response_fifo_full_i    => s_fif_wr_full,       -- response fifo full flag
    response_ready_o        => opc_rsp_ready,       -- response is waiting
    response_length_o       => open, --opc_rsp_len,         -- update response length when response is ready
    response_fifo_count_i   => s_fif_dat_wr_level,  -- response fifo count, opcode processor asserts 
                                                    -- response_ready when fifo_length==response_length
                                                    -- seems overly complex...
  
--    frequency_o             => frequency,
--    frq_wr_en_o             => freq_fifo_wr_en,
